...
class StaticController implements ng.IComponentController {
    constructor(private $stateParams: { page: string }, private $ceo: CEOService) { }

    public $onInit() {
        this.setCeo(this.$stateParams.page);
    }

    public setCeo(page: string) {
...
StaticController.$inject = ['$stateParams', CEOService_NAME];
